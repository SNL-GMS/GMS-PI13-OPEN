...
if [[ "$ENABLE_METRICS" == "on" ]]; then
    export KAFKA_OPTS="$KAFKA_OPTS -javaagent:/opt/kafka/prometheus/jmx_prometheus_javaagent.jar=$METRICS_PORT:/opt/kafka/prometheus/kafka.yml"
fi
