...
public class Cd11AcquiredChannelEnvironmentIssuesParser {
   ...
   * @param chanName Channel name of the subframe
   ...
   */
  public static List<AcquiredChannelEnvironmentIssue> parseAcquiredChannelSoh(byte[] fields,
      String chanName,
      Instant startTime, Instant endTime) {
    logger.info("Parsing subframe for channel: {}, time range: {} - {}", chanName, startTime,
        endTime);
    ...
    if (fields.length != 32) {
      ...
      // Data status byte
      idx = 1;
      stateOfHealthSet.addAll(parseDataStatusByte(fields[idx], chanName, startTime, endTime));

      // Channel security byte
      idx = 2;
      stateOfHealthSet.addAll(parseChannelSecurityByte(fields[idx], chanName, startTime, endTime));

      // Miscellaneous status byte
      idx = 3;
      stateOfHealthSet.addAll(parseMiscStatusByte(fields[idx], chanName, startTime, endTime));

      // Voltage indicator byte
      idx = 4;
      stateOfHealthSet.addAll(parseVoltageIndicatorByte(fields[idx], chanName, startTime, endTime));

      // Clock differential in microseconds.
      idx = 28;
      ByteBuffer bb = ByteBuffer.wrap(fields, idx, 4);
      stateOfHealthSet.add(AcquiredChannelEnvironmentIssueAnalog.create(chanName,
  ...
  private static List<AcquiredChannelEnvironmentIssue> parseDataStatusByte(byte field,
      String chanName, Instant startTime, Instant endTime) {
    List<AcquiredChannelEnvironmentIssue> parsedSohSet = new ArrayList<>();
    parsedSohSet.add(AcquiredChannelEnvironmentIssueBoolean.create(chanName,
  ...
  private static List<AcquiredChannelEnvironmentIssue> parseChannelSecurityByte(byte field,
      String chanName, Instant startTime, Instant endTime) {
    List<AcquiredChannelEnvironmentIssue> parsedSohSet = new ArrayList<>();
    parsedSohSet.add(AcquiredChannelEnvironmentIssueBoolean.create(chanName,
  ...
  private static List<AcquiredChannelEnvironmentIssue> parseMiscStatusByte(byte field,
  ...
  private static List<AcquiredChannelEnvironmentIssue> parseVoltageIndicatorByte(byte field,
      String chanName, Instant startTime,
      Instant endTime) {
    List<AcquiredChannelEnvironmentIssue> parsedSohSet = new ArrayList<>();
    parsedSohSet.add(AcquiredChannelEnvironmentIssueBoolean.create(chanName,
