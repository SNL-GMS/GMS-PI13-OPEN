...
class Cd11ConnectionRequestFrameTest {
  ...
  private static ByteBuffer initConnReq() throws UnknownHostException {
  ...
  @Test
  void testConnReqParsing() throws IOException, InterruptedException {
    ...
    assertEquals(IP, InetAddresses.toAddrString(InetAddresses.fromInteger(requestFrame.ipAddress)));
    assertEquals(PORT, requestFrame.port);
    assertEquals(IP2, InetAddresses.toAddrString(InetAddresses.fromInteger(requestFrame.secondIpAddress)));
