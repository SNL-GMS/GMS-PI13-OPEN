...
class Cd11ConnectionResponseFrameTest {
  ...
  private static ByteBuffer initConnResp() throws UnknownHostException {
  ...
  @Test
  void testConnRespParsing() throws IOException, InterruptedException {
    ...
    assertEquals(IP, InetAddresses.toAddrString(InetAddresses.fromInteger(responseFrame.ipAddress)));
    assertEquals(PORT, responseFrame.port);
    assertEquals(IP2, InetAddresses.toAddrString(InetAddresses.fromInteger(responseFrame.secondIpAddress)));
