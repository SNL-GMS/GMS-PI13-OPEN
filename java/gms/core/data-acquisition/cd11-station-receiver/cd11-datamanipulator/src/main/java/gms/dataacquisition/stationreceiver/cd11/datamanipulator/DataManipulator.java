...
public class DataManipulator {
    ...
    private static ReadStationDataFrame convertRawToRead(RawStationDataFrame rawStationDataFrame)
        ...
        Cd11ByteFrame byteFrame = new Cd11ByteFrame(
            new DataInputStream(new ByteArrayInputStream(rawStationDataFrame.getRawPayload())),
            () -> true);

        Cd11DataFrameSoh cd11Frame = new Cd11DataFrameSoh(byteFrame);
        return new ReadStationDataFrame(cd11Frame, rawStationDataFrame);
    }
     ...
     * @throws IOException when toBytes fails from the cd11dataframe
     */
    private static RawStationDataFrame convertReadToRaw(ReadStationDataFrame readStationDataFrame) throws IOException {
        // this creates a new Cd11DataFrame using the soh strings as the truth, and changes the bytes to match
        ...
        Cd11DataFrame newCd11DataFrame = readStationDataFrame.getCd11DataFrameSoh().transcribeSohToBytes();
        ...
        return RawStationDataFrame.builder()
                ...
                .setRawPayload(newCd11DataFrame.toBytes())
                .build();
    }

    private static List<RawStationDataFrame> convertReadToRaw(List<ReadStationDataFrame> readStationDataFrames) throws IOException {
