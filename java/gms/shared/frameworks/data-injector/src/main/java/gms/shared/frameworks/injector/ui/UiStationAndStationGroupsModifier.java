 ...
 */
public class UiStationAndStationGroupsModifier implements Modifier<Iterable<UiStationAndStationGroups>> {
  ...
  private EmitterProcessor<SystemMessage> systemMessageEmitterProcessor = EmitterProcessor.create();
  ...
  public UiStationAndStationGroupsModifier() {
    ...
    this.sohQuietAndUnacknowledgedCacheManager =
        new SohQuietAndUnacknowledgedCacheManager(
            ...
            Collections.emptyList(), // empty StationSoh collection
  ...
  @Override
  public List<UiStationAndStationGroups> apply(Iterable<UiStationAndStationGroups> uiStationAndStationGroupsSohIterable) {
    List<StationSoh> newUpdatedStations = this.stationSohGenerator.getUpdatedStations();

    List<StationSoh> newStationList =
        StreamSupport.stream(newUpdatedStations.spliterator(), false).collect(Collectors.toList());
    // Update unack with updated stations
    this.sohQuietAndUnacknowledgedCacheManager.updateUnacknowledgedList(newStationList);
    ...
    List<UiStationAndStationGroups> generatedUiStationAndStationGroupsSohList =
        UiStationAndStationGroupGenerator.generateUiStationAndStationGroups(
            newUpdatedStations,
            this.sohQuietAndUnacknowledgedCacheManager.getUnacknowledgedList(),
    ...
    return generatedUiStationAndStationGroupsSohList;
  }
  ...
  public void generateAndPublishUiStationSoh(List<String> stationNames) {
    List<QuietedSohStatusChangeUpdate> quietedSohStatusChanges =
        this.sohQuietAndUnacknowledgedCacheManager.getQuietedSohStatusChanges();
    List<StationSoh> latestStationSohList =
        this.sohQuietAndUnacknowledgedCacheManager.getLastStationSohList();
    List<StationSoh>stationsToUpdate = !stationNames.isEmpty() ? latestStationSohList.stream().
        filter(s -> stationNames.contains(s.getStationName())).collect(Collectors.toList()) : latestStationSohList;
