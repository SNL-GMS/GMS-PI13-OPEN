  ...
  private HttpResponse<String> performHTTP2Upgrade(ServiceRequest request) {
    try {
      return httpClient.send(HttpRequest
          .newBuilder(
              URI.create(String.format("%s://%s:%d" + GmsCommonRoutes.CONNECTION_UPGRADE_PATH,
                  request.getUrl().getProtocol(),
                  request.getUrl().getHost(), request.getUrl().getPort())))
          .GET()
          .build(), BodyHandlers.ofString());
    } catch (IOException | InterruptedException e) {
      throw new ConnectionFailed("Could not upgrade connection to " + request.getUrl().getHost(),
          e);
    }
  }
