...
public enum SohMonitorType {
  ...
  ENV_CLOCK_DIFFERENTIAL_IN_MICROSECONDS((short)10, SohValueType.PERCENT),
  ...
  ENV_STATION_POWER_VOLTAGE((short)55),
  ...
  private static Set<SohMonitorType> validTypes = Arrays.stream(SohMonitorType.values())
      .filter(type -> !type.getSohValueType().equals(SohValueType.INVALID)).collect(
      Collectors.toSet());

  private short dbId;
  private boolean isEnvironmentIssue;
  private SohValueType sohValueType;
