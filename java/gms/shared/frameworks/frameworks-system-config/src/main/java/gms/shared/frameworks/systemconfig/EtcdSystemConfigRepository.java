...
public class EtcdSystemConfigRepository implements SystemConfigRepository {
  ...
  private static final RetryPolicy<Optional<String>> ETCD_RETRY_POLICY
      = new RetryPolicy<Optional<String>>()
      .withBackoff(50, 1000, ChronoUnit.MILLIS)
      .withMaxAttempts(100)
      .handle(Exception.class)
      .onFailedAttempt(e -> logger.warn("Failed connecting to etcd, will try again..."));
  ...
  /**
   * Etcd-specific implementation of get.  Performs retries if
   * etcd is unavailable.
   *
  ...
  public Optional<String> get(String key) {
    return Failsafe.with(ETCD_RETRY_POLICY)
        .onFailure(e -> logger.error("Etcd unavailable!  Giving up"))
        .get(() -> fetchEtcd(key));
  }

  private Optional<String> fetchEtcd(String key) {
    try {
      final RangeResponse response = this.etcdClientManager.getEtcdClient()
        .get(ByteString.copyFromUtf8(key)).sync();
      final long responseCount = response.getCount();
      if (responseCount > 0) {
        if (responseCount > 1) {
          logger.warn(
      ...
      logger.error("Error connecting to ETCD", ex.getMessage());
  ...
  public static class Builder {
    ...
    private static final String DEFAULT_ETCD_PASSWORD  = "gmsdb:etcd:gms:packager-tungsten-sort";
