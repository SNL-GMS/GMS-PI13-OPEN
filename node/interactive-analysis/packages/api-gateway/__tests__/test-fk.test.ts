...
import { PhaseType } from '../src/ts/common/model';
...
describe('fk util getDefaultFkConfigurationForSignalDetection', () => {
  test('should return expected medium velocity for phase', () => {
    ...
    expect(getDefaultFkConfigurationForSignalDetection(pPhase, '1').mediumVelocity).toBeCloseTo(
      expectedForP
    );
    expect(getDefaultFkConfigurationForSignalDetection(sPhase, '1').mediumVelocity).toBeCloseTo(
      expectedForS
    );
    expect(getDefaultFkConfigurationForSignalDetection(lgPhase, '1').mediumVelocity).toBeCloseTo(
      expectedForLg
    );
    expect(getDefaultFkConfigurationForSignalDetection(rgPhase, '1').mediumVelocity).toBeCloseTo(
      expectedForRg
    );
    expect(getDefaultFkConfigurationForSignalDetection(otherPhase, '1').mediumVelocity).toBeCloseTo(
      expectedForOther
    );
  });
});
