...
import { WorkflowProcessor } from '../src/ts/workflow/workflow-processor';
...
it('Opening a ProcessingStageInterval should match snapshot', async () => {
  ...
  mutation openStageWIntervals{
    markStageInterval(stageIntervalId: "24a1dd29-6ada-47a5-af1f-a0883d570a5c", input: { status: InProgress }){
      status
      activityIntervals{
        status
        activeAnalysts{
...
it('Marking a ProcessingStageInterval Complete should match snapshot', async () => {
  ...
  mutation completeStageInterval{
    markStageInterval(stageIntervalId: "d2e1af21-5554-4bc7-b089-29bf92c2c95e", input: {status: Complete }){
      status
      completedBy{
        userName
      }
...
it('Marking a ProcessingActivityInterval Complete should match snapshot', async () => {
  ...
  mutation completeActivityInterval{
    markActivityInterval(activityIntervalId: "fd1520fe-7721-4c52-aab5-ab401d9f9fd4", input: {status: Complete }){
      activityInterval {
        id
        status
        completedBy{
...
it('Changing stage status to NotComplete should result in an error', async () => {
  ...
  const mutation = `
  mutation m{
    markStageInterval(stageIntervalId: "79f0afe0-917f-4fa1-8187-f8c0b03dd98c", input: {status: NotComplete }){
      id
      status
    }
  }
  ...
  // Execute the GraphQL query
  const result = await graphql(schema, mutation, {}, userContext);
  const { errors } = result;
...
it('Changing stage status from NotStarted to Complete should result in an error', async () => {
  ...
  const mutation = `
  mutation m{
    markStageInterval(stageIntervalId: "79f0afe0-917f-4fa1-8187-f8c0b03dd98c", input: {status: Complete }){
      id
      status
    }
  }
...
it('Changing activity status from NotStarted to Complete should result in an error', async () => {
  ...
  const mutation = `
  mutation m{
    markActivityInterval(activityIntervalId: "79f0afe0-917f-4fa1-8187-f8c0b03dd98c", input: { status: Complete }){
      id
      status
    }
  }
