...
export function initialize(httpMockWrapper: HttpMockWrapper) {
  ...
  httpMockWrapper.onMock(
    backendConfig.services.masksByChannelIds.requestConfig.url,
    getQcMasksByChannelIds
  );
...
function getQcMasksByChannelIds(input: any): any {
  ...
  const startTime: number = toEpochSeconds(input.startTime);
  const endTime: number = toEpochSeconds(input.endTime);
  const channelIds = input.channelNames;
  const mapEntries = {};
  channelIds.forEach(chanId => {
    const masks = filter(dataStore.qcMasks, mask => {
      ...
      if (mask.channelName === chanId && currStart < endTime && currEnd > startTime) {
        return true;
      }
    });
    if (masks && masks.length > 0) {
      mapEntries[chanId] = masks;
    }
  });
  return mapEntries;
}
...
function loadTestData(): QcMaskDataStore {
  ...
  const dataPaths: string[] = [
    ...
    [paths.additionalDataHome, additionalDataConfig.qcMaskFileName]
  ].map(dataPath => {
  ...
  logger.info(`Loaded QC Masks count: ${qcMasks.length}`);
  return {
    qcMasks
  };
}
