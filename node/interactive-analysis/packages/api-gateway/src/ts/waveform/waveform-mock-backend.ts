...
import { ChannelSegmentType, OSDChannelSegment, TimeSeriesType } from '../channel-segment/model';
import { gatewayLogger as logger } from '../log/gateway-logger';
import { HttpMockWrapper } from '../util/http-wrapper';
import { resolveTestDataPaths } from '../util/test-data-util';
import { MockWaveform, OSDChannelCalibration, OSDWaveform } from './model';

/**
 * Waveform data store
 */
interface WaveformDataStore {
  calibrationMap: Map<string, OSDChannelCalibration[]>;
}
...
function loadTestData(): WaveformDataStore {
  ...
  const calibrationFile = dataPath
    .concat(path.sep)
    .concat(config.get('testData.additionalTestData.channelCalibrationFile'));
  const calibrationMap: Map<string, OSDChannelCalibration[]> = new Map();

  try {
    // Load the calibrations if that is not empty build the map
    const calibrations: OSDChannelCalibration[] = readJsonData(calibrationFile);
    if (calibrations) {
      calibrations.forEach(cal => {
        if (calibrationMap.get(cal.channelName) === undefined) {
          calibrationMap.set(cal.channelName, [cal]);
        } else {
          calibrationMap.get(cal.channelName).push(cal);
        }
      });
    }
  } catch (e) {
    logger.error(`Failed to load calibration file: ${calibrationFile}`);
  }
 ...
 * @param channelName a channel ID
 */
export function getCalibrationFactor(channelName: string, startTime: number): number {
  const calibrations = dataStore.calibrationMap.get(channelName);

  if (calibrations) {
    let actualTime = 0; // jan 1 1970
    let calibrationFactor;
    calibrations.forEach(cal => {
      const calActualTime = toEpochSeconds(cal.actualTime);
      if (calActualTime > actualTime && calActualTime < startTime) {
        actualTime = calActualTime;
        calibrationFactor = cal.calibrationFactor;
      }
    });

    // If set return the cal factor
    if (calibrationFactor) {
      return calibrationFactor;
    }
  }
  return 1; // return default factor
}
...
function createWaveformSegments(
  startTime: number,
  channelSegments: OSDChannelSegment<MockWaveform>[],
  populateSamples: SamplePopulator
  ...
  channelSegments.forEach(segment => {
    const newSegment: OSDChannelSegment<OSDWaveform> = {
      ...
      timeseries: segment.timeseries.map(mockWaveform => {
        populateSamples(mockWaveform, startTime, segment.type, mockWaveform.dataType);
        return {
          startTime: segment.startTime,
          sampleCount: mockWaveform.sampleCount,
          sampleRate: mockWaveform.sampleRate,
          type: mockWaveform.type,
...
export function getWaveformSegmentsByChannelSegments(
  ...
  if (waveformMode.toLowerCase() === 'simulated') {
    return createWaveformSegments(startTime, channelSegments, generateMockWaveformSamples);
    // If configured for data 'fromFile', read waveforms from the configured files
  }

  if (waveformMode.toLowerCase() === 'fromfile') {
    return createWaveformSegments(startTime, channelSegments, readWaveformSamples);
  }
...
function readWaveformSamples(
  ...
  waveform.values = readWaveformSamplesFromFile(
    waveformFile,
    waveform.fOff,
    waveform.sampleCount * FOUR_BYTES,
    segmentType,
    dataType
  );
}
...
function readWaveformSamplesFromFile(
  ...
  const byteBuffer = Buffer.alloc(readLengthInBytes);
  ...
  // FIXME- need to agree on the Endianness of the .w data to be consistent
  ...
  if (dataType === 'f4') {
    // segmentType === ChannelSegmentType.FK_BEAM) {
    for (let i = 0; i < bytesRead; i += FOUR_BYTES) {
      samples[i / FOUR_BYTES] = byteBuffer.readFloatBE(i);
    }
  } else if (dataType === 's4') {
    for (let i = 0; i < bytesRead; i += FOUR_BYTES) {
      samples[i / FOUR_BYTES] = byteBuffer.readInt32BE(i);
    }
  } else {
 ...
 * @param eventAmplitude the event amplitude
...
function createMockSamples(
  ...
  for (let i = 1; i < samples; i++) {
    ...
    data[i - 1] =
      ...
      noiseAmplitude -
      getSecureRandomNumber() * 2 * noiseAmplitude -
