...
let workflowCache: model.WorkflowData;
...
function loadTestData() {
  ...
  // Read the workflow-related data sets from JSON files
  workflowCache = readJsonData(
    dataPath.concat(path.sep).concat(testDataConfig.workflowFilename)
  )[0];
}
 ...
 */
export async function getWorkflowData(): Promise<model.WorkflowData> {
