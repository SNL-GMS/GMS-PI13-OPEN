...
describe('history utils', () => {
  ...
  test('can get the index of the last of type for an event', () => {
    expect(Utils.getIndexOfLastOfTypeForEvent).toBeDefined();

    const testParams = [historyList, firstEventId, HistoryEntryAction.undo];
    testPermutationsUndefined(Utils.getIndexOfLastOfTypeForEvent, testParams);

    expect(
      Utils.getIndexOfLastOfTypeForEvent(historyList, firstEventId, HistoryEntryAction.undo)
    ).toBeGreaterThan(-1);
    expect(
      Utils.getIndexOfLastOfTypeForEvent(historyList, firstEventId, HistoryEntryAction.redo)
    ).toEqual(-1);
    expect(
      Utils.getIndexOfLastOfTypeForEvent(historyList, 'undefined event', HistoryEntryAction.undo)
    ).toEqual(-1);
  });
  ...
  test('can get history action type from history', () => {
    ...
    expect(Utils.isUndo(historyEntry)).toEqual(true);
    expect(Utils.isRedo(historyEntry)).toEqual(false);
  });
  ...
  test('can determine if an entry caused a conflict', () => {
    expect(Utils.isResponsibleForConflict).toBeDefined();

    expect(Utils.isResponsibleForConflict(historyList[2])).toEqual(true);
    expect(Utils.isResponsibleForConflict(historyList[1])).toEqual(false);
  });
  ...
  test('can filter out irrelevant changes', () => {
    ...
    expect(Utils.filterIrrelevantChanges(historyList[1]).changes.length).toEqual(
  ...
  test('can format create event history', () => {
    ...
    expect(formattedEntry.changes.length).toEqual(historyList[1].changes.length - 1);
