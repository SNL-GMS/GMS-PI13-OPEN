...
/**
 * Gets the last index that matches the type and event id provided
 * @param historyList a list of History entries to check
 * @param eventId the event ID to check against
 */
export function getIndexOfLastOfTypeForEvent(
  historyList: CacheTypes.History[],
  eventId: string,
  entryType: HistoryEntryAction
): number | undefined {
  if (historyList === undefined || entryType === undefined || !eventId) {
    return undefined;
  }
  ...
  while (
    ...
    (getHistoryActionType(historyList[lastIndex]) !== entryType ||
      !doesEntryAffectEvent(historyList[lastIndex], eventId))
  ) {
    lastIndex--;
  }
  return lastIndex;
...
/**
 * Check if a history entry entry can be undone
 * @param entry a History to check
 * @returns whether a history entry entry can be undone
 */
export function isUndo(entry: CacheTypes.History): boolean {
  return getHistoryActionType(entry) === HistoryEntryAction.undo;
}

/**
 * Check if a history entry entry can be redone
 * @param entry a History to check
 * @returns whether a history entry entry can be redone
 */
export function isRedo(entry: CacheTypes.History): boolean {
  return getHistoryActionType(entry) === HistoryEntryAction.redo;
}
...
/**
 * Check if a history entry entry created a conflict
 * @param entry a history entry entry to check if that action was responsible for creating a conflict.
 * @returns whether a conflict was created by at least one of the changes in a history entry
 */
export function isResponsibleForConflict(entry: CacheTypes.History): boolean {
  return entry.changes.reduce(
    (isInConflict, change) => isInConflict || change.conflictCreated,
    false
  );
}
