...
import { MILLISECONDS_IN_SECOND, timeSecondsToISOString } from '@gms/common-util';
...
export class LocationSignalDetections extends React.Component<LocationSignalDetectionsProps> {
  ...
  public render() {
    ...
    const azimuthNoneDefining = mainTableRowData
      .map(row => row.azimuthDefining)
      .reduce((accumulator, currentValue) => !currentValue && accumulator, true);
    const aziumthDefiningState: DefiningStates = azimuthAllDefining
      ? DefiningStates.ALL
      : azimuthNoneDefining
      ? DefiningStates.NONE
      : DefiningStates.SOME;
    return (
      <div className={classNames('ag-theme-dark', 'table-container', Classes.DARK)}>
        <div className="list-wrapper">
          <div className={'max'}>
            <Table
              id="table-location-signal-detections"
              key="table-location-signal-detections"
              context={{}}
              columnDefs={generateLocationSDColumnDef(
                this.props.setDefining,
                timeDefiningState,
                aziumthDefiningState,
  ...
  private readonly generateTableRows = (
    ...
    signalDetections.map(sd => {
      ...
      return {
        ...
        // TODO: Also stripChannelName returns empty channel name
        channel: 'fkb',
        phase: sd.phase,
        distance: distance ? distance.degrees : undefined,
        timeObs: timeSecondsToISOString(new Date(sd.time.observed * MILLISECONDS_IN_SECOND)),
