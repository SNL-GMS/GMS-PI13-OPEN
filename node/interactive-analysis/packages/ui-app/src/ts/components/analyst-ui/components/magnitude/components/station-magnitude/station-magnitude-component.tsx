...
> {
   ...
   */
  public componentDidUpdate(prevProps: StationMagnitudeProps, prevState: StationMagnitudeState) {
    ...
    if (prevProps.openEventId !== this.props.openEventId) {
      ...
      if (!hasStatus) {
        this.setState({
          computeNetworkMagnitudeSolutionStatus: this.state.computeNetworkMagnitudeSolutionStatus.set(
            this.props.openEventId,
            undefined
          )
        });
    ...
    return (
      <div className={classNames('ag-theme-dark', 'table-container')}>
        <div className="list-wrapper">
          <div className={'max'}>
            <Table
              id="table-station-magnitude"
              key="table-station-magnitude"
  ...
  private readonly onShiftClick = (params: any, magType: EventTypes.MagnitudeType) => {
    ...
    const data: StationMagnitudeRow = params.data;
  ...
  private readonly getSelectedRowIndicesForPhase = (
    ...
    if (this.mainTable) {
      this.mainTable.getRenderedNodes().forEach(node => {
        const data: StationMagnitudeRow = node.data;
