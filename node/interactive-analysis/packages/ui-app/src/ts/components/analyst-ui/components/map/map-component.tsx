...
import { CESIUM_OFFLINE, dateToISOString } from '@gms/common-util';
...
export class Map extends React.PureComponent<MapProps, MapState> {
  ...
  public componentDidUpdate(prevProps: MapProps) {
    ...
    if (!isEqual(currentOpenEvent, previousOpenEvent)) {
      ...
      if (!isEqual(currentSignalDetectionsByStation, prevSignalDetectionsByStation)) {
        this.map.drawSignalDetections(
          currentSignalDetectionsByStation,
          currentEventsInTimeRange,
          currentOpenEvent,
  ...
  private readonly mapTopOptions = (): JSX.Element => (
    <div className="map-controls">
      ...
      <label className={`${Classes.LABEL} ${Classes.INLINE} map-controls__label`}>
        Start time: <span className="map-time">{dateToISOString(new Date())}</span>
      </label>
      <label className={`${Classes.LABEL} ${Classes.INLINE} map-controls__label`}>
        End time: <span className="map-time">{dateToISOString(new Date())}</span>
      </label>
    </div>
  )
  ...
  private readonly toggleDataLayerVisibility = (id: string, show?: boolean) => {
    ...
    if (layer) {
      show == undefined ? (layer.show = !layer.show) : (layer.show = show);
      this.forceUpdate();
    }
  }
