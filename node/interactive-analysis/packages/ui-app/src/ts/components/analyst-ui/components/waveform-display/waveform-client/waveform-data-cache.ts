...
export class WaveformDataCache {
  ...
  public readonly getWaveformEntry = (
    ...
    this.cache.has(channelId)
      ? this.cache.get(channelId).has(filterId)
        ? this.cache.get(channelId).get(filterId)
        : undefined
      : undefined
  ...
  public readonly clearAllWaveformEntry = (channelId: string, filterId?: string) => {
    if (channelId) {
      if (this.cache.has(channelId)) {
        if (filterId) {
          this.cache.get(channelId).delete(filterId);
        } else {
          this.cache.get(channelId).clear();
    ...
    if (channelId) {
      ...
      if (filterId) {
        ...
        if (existingData) {
          ...
          this.cache.get(channelId).set(filterId, [value]);
        }
        return value;
      }
    }
