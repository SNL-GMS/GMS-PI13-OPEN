import { dateToShortISOString, MILLISECONDS_IN_SECOND } from '@gms/common-util';
import GoldenLayout from '@gms/golden-layout';
import { HistoryList, HistoryListTypes, Toolbar, ToolbarTypes } from '@gms/ui-core-components';
...
export class WorkflowMenuBar extends React.Component<WorkflowMenuBarProps, WorkflowMenuBarState> {
  ...
  public componentDidUpdate(prevProps: WorkflowMenuBarProps, prevState: WorkflowMenuBarState) {
    if (
      this.props.startTimeSecs !== prevProps.startTimeSecs ||
  ...
  public render() {
    ...
    const expandCollapse: ToolbarTypes.ButtonItem = {
    ...
    const intervalPicker: ToolbarTypes.IntervalPickerItem = {
      ...
      onChange: (startDate: Date, endDate: Date) => {
        this.onNewInterval(startDate, endDate);
      },
    ...
    const intervalDropdown: ToolbarTypes.PopoverItem = {
      label: 'Intervals',
    ...
    toolbarItems.push(expandCollapse);
    toolbarItems.push(intervalPicker);
    toolbarItems.push(intervalDropdown);
    ...
    return (
      <div className="workflow__menu-bar">
        <Toolbar
          ...
          toolbarWidthPx={
            this.props.glContainer ? this.props.glContainer.width - SIZE_OF_MENU_BAR_PADDING_PX : 0
          }
          items={toolbarItems}
        />
      </div>
  ...
  private readonly applyNewInterval = (value: any) => {
    ...
    if (
      value.startDate.valueOf() / MILLISECONDS_IN_SECOND !== this.props.startTimeSecs ||
      value.endDate.valueOf() / MILLISECONDS_IN_SECOND !== this.props.endTimeSecs
    ) {
      // Try to find new interval in the history
      const historyList = this.state.history;
      const maybeEntry = historyList.find(
        his =>
          his.startDate.valueOf() === this.props.startTimeSecs * MILLISECONDS_IN_SECOND &&
          his.endDate.valueOf() === this.props.endTimeSecs * MILLISECONDS_IN_SECOND
      );
      if (maybeEntry) {
        // If found, reshuffles the history entries
        const listWithoutEntry = historyList.filter(
          his =>
            his.startDate.getTime() !== maybeEntry.startDate.getTime() ||
            his.endDate.getTime() !== maybeEntry.endDate.getTime()
        );
        ...
        this.setState(
          { startDate: value.startDate, endDate: value.endDate, history: newList },
          () => {
        ...
        const startDate = new Date(this.props.startTimeSecs * MILLISECONDS_IN_SECOND);
        const endDate = new Date(this.props.endTimeSecs * MILLISECONDS_IN_SECOND);
        this.counter++;
        historyList.push({
        ...
        this.setState(
          { startDate: value.startDate, endDate: value.endDate, history: historyList },
          () => {
            this.props.onNewInterval(value.startDate, value.endDate);
          }
        );
  ...
  /**
   * Sets state for the interval picker - does not call gateway
  ...
  private readonly onHistoryItemSelect = (id: string) => {
    if (id === currentTimeId) {
      this.applyNewInterval({
        startDate: this.state.currentTime.startDate,
  ...
  private readonly generateLabelForHistory = (startDate: Date, endDate: Date) => {
    const reactLabel = (
      <span className="history-item">
        <span className="history-item__date-string">{dateToShortISOString(startDate)}</span>
        <span className="history-item__text">&nbsp;to&nbsp;</span>
        <span className="history-item__date-string">{dateToShortISOString(endDate)}</span>
      </span>
    );
    return reactLabel;
  }
