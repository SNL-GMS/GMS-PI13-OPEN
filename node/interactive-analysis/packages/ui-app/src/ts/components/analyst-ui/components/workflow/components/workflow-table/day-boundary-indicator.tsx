...
export class DayBoundaryIndicator extends React.Component<DayBoundaryIndicatorProps, {}> {
  ...
  public render() {
    ...
    const rowWidthPx = Math.ceil((totalDurationSecs / SECONDS_PER_HOUR) * this.props.pixelsPerHour);

    const dividers = d3.utcDay.every(1).range(
      moment
    ...
    return dividers.map(date => {
      ...
      const left = `calc(${((date.valueOf() / MILLISECONDS_IN_SECOND - this.props.startTimeSecs) /
  ...
  public readonly scrollDayIndicator = (scrollTo: number) => {
    ...
    this.dayIndicators.forEach(
      di => (di.style.marginLeft = (-scrollTo + marginOfIntervalTable).toString() + 'px')
    );
  }
}
