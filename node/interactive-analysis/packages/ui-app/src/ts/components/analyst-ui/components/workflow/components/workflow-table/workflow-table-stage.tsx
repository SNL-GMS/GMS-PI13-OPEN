 ...
 */
export class WorkflowTableStage extends React.Component<WorkflowTableStageProps> {
  /**
   * Constructor.
   *
        ...
        <div>
          <div className="stage-row__sub-row">
            <WorkflowRow
              ...
              currentlySelectedIntervalId={this.props.currentlySelectedIntervalId}
              showStageIntervalContextMenu={this.props.showStageIntervalContextMenu}
          ...
          {this.props.stage.activities.length > 0 && this.props.isExpanded
            ? this.props.stage.activities.map(activity => (
                <div key={activity.name + this.props.stage.id} className="stage-row__sub-row">
                  <WorkflowRow
                    stage={this.props.stage}
                    currentlySelectedIntervalId={this.props.currentlySelectedIntervalId}
                    showActivityIntervalContextMenu={this.props.showActivityIntervalContextMenu}
                    activity={activity}
                    markActivityInterval={this.props.markActivityInterval}
                  />
                  <WorkflowRowLabel label={activity.name} isActivityRow={true} isExpanded={false} />
                </div>
              ))
            : undefined}
        </div>
