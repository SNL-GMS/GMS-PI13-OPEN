...
> {
  ...
  private table: InfiniteTable<{ id: string; severity: string }, {}>;
    ...
    return (
      ...
      >
        <this.TableWithDatasource>
          <InfiniteTable<SystemMessageInfiniteTableDataType, {}>
        ...
        <NewMessageIndicator
          ...
          handleNewMessageIndicatorClick={e => this.onNewMessageIndicatorClick()}
        />
      </div>
    );
  }
  ...
  > = props => {
    ...
    if (React.isValidElement(child)) {
       ...
       */
      return <React.Fragment>{React.cloneElement(child, { datasource })}</React.Fragment>;
    }
  }
   ...
   * @param params the event parameters
   */
  private readonly onSortChanged = (params: SortChangedEvent): void => {
   ...
   */
  private readonly onBodyScroll = (event: BodyScrollEvent): void => {
    this.trackScrollDelta();
   ...
   */
  private readonly timeValueGetter: NumberValueGetter = (params: StringValueGetterParams): number =>
    this.state.systemMessages?.find(msg => params.data && msg?.id === params.data?.id)?.time
  ...
  private readonly onCellClicked = (
    params: CellClickedEvent<{ id: string; severity: string }, {}, {}>
  ): void => {
  ...
  private readonly onCellContextMenu = (
    params: CellContextMenuEvent<{ id: string; severity: string }, {}, {}>
  ): void => {
    return;
  }
}
