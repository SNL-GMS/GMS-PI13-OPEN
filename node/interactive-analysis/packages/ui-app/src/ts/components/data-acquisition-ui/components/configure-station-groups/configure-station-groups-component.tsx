...
import {
  ConfigureStationGroupsProps,
  ConfigureStationGroupsState,
  FilterType,
  NetworkRow,
  StationsRow,
  StationsSelectionState
} from './types';

export class ConfigureStationGroups extends React.PureComponent<
  ConfigureStationGroupsProps,
  ConfigureStationGroupsState
> {
  ...
  public static generateTableData = (props: ConfigureStationGroupsProps): StationsRow[] => {
    ...
    if (defaultStations) {
      ...
      availableStationListData = sortBy(availableStationListData, 'stations');
    }
  ...
  public render() {
    ...
    return (
      <div className={classNames('ag-theme-dark', 'table-container-hide-overflow')}>
        <div className={'list-wrapper'}>
          <div className={'configure-station-groups-select-network'}>
            ...
            <div className={'configure-station-groups-select-network-table'}>
              <Table
                id="table-configure-station-groups-select-network-table"
                key="table-configure-station-groups-select-network-table"
                context={{}}
          ...
          >
            <div className={'configure-station-groups-associated-stations'}>
              <Table
                id="table-configure-station-groups-associated-stations"
                key="table-configure-station-groups-associated-stations"
                context={{}}
                ...
                rowData={
                  selectedNetworkData && selectedNetworkData.associatedStationsTableData
            ...
            <div className={'configure-station-groups-available-stations'}>
              <Table
                id="table-configure-station-groups-available-stations"
                key="table-configure-station-groups-available-stations"
                context={{}}
                ...
                rowData={
                  selectedNetworkData && selectedNetworkData.availableStationsTableData
        ...
        <ButtonGroup>
          <Button small={true} text="Add group" onClick={e => this.newNetwork()} />
          <div className={'list__button-row-divider-small'} />
          <Button small={true} text="Deactivate group" onClick={e => this.deactivateNetwork()} />
          <div className={'list__button-row-divider-large'} />
          <Button small={true} text="Save" onClick={e => this.saveState()} />
          <div className={'list__button-row-divider-small'} />
          <Button small={true} text="Discard" onClick={e => this.discardState()} />
        </ButtonGroup>
      </div>
    );
  }
  ...
  private readonly filterTableData = (data: NetworkRow[]): NetworkRow[] => {
    ...
    switch (this.state.selectedFilter) {
      case 'All networks':
        ...
        break;
      case 'Active':
        filteredList = data.filter(row => row.status === 'Active');
        break;
      case 'Inactive':
        filteredList = data.filter(row => row.status === 'Inactive');
        break;
      default:
    }
    return filteredList;
  ...
  private readonly onRowClicked = (event: any) => {
    ...
    if (selectedStationIds.length === 1) {
      ...
      const mapState = cloneDeep(this.state.networkToStationsMap);
  ...
  private readonly setModified = (id: string, value: boolean) => {
    this.gridApiNetworks.getRowNode(id).setDataValue('modified', value);
  }
  ...
  private readonly getStationsSelectionState = (id: string): StationsSelectionState => {
    const selectedNetworkData: StationsSelectionState = this.state.networkToStationsMap.has(id)
  ...
  private readonly updateAssocHeader = () => {
    ...
    const curNetId: string = this.state.currentNetworkID
      ? this.state.currentNetworkID
      : `default_${this.rowId}`;
    if (
      this.gridApiNetworks.getDisplayedRowCount() > 0 &&
      ...
      this.gridApiAssoc.getColumnDef('associated_stations').headerName =
        newNetworkName + `: ${rowCount} ${stationHeader}`;
  ...
  private readonly unassociateStations = () => {
    const selectedNetworkStationStates: StationsSelectionState = this.getStationsSelectionState(
      this.state.currentNetworkID
    );
    ...
    let updatedAvailableTableData: StationsRow[] = [
      ...selectedNetworkStationStates.availableStationsTableData,
      ...selectedRows
    ];
    updatedAvailableTableData = sortBy(updatedAvailableTableData, 'stations');
    // Remove selected rows from associated stations
    const updatedAssociatedTableData: StationsRow[] = selectedNetworkStationStates.associatedStationsTableData.filter(
      row => !includes(updatedAvailableTableData, row)
    );
    // Create the stations state
    const newStationsState: StationsSelectionState = {
      associatedStationsOriginalTableData:
        selectedNetworkStationStates.associatedStationsOriginalTableData,
      associatedStationsTableData: updatedAssociatedTableData,
      availableStationsOriginalTableData:
        selectedNetworkStationStates.availableStationsOriginalTableData,
  ...
  private readonly associateStations = () => {
    ...
    let updatedAssociatedTableData: StationsRow[] = [
      ...selectedNetworkStationStates.associatedStationsTableData,
      ...selectedRows
    ];
    updatedAssociatedTableData = sortBy(updatedAssociatedTableData, 'stations');
    const updatedAvailableTableData: StationsRow[] = selectedNetworkStationStates.availableStationsTableData.filter(
      row => !includes(updatedAssociatedTableData, row)
    );
    // Create the stations state
    const newStationsState: StationsSelectionState = {
      associatedStationsOriginalTableData:
  ...
  private readonly newNetwork = () => {
    const defaultRow: NetworkRow = {
      ...
      status: 'Active',
      prevStatus: 'Active',
      modifiedTime: dateToISOString(new Date())
    };
  ...
  private readonly deactivateNetwork = () => {
    ...
    const newNetworkRows: NetworkRow[] = cloneDeep(this.state.networkTableData);
  ...
  private readonly saveState = () => {
    const selectedRows: NetworkRow[] = this.gridApiNetworks.getSelectedRows();
    const mapState: Map<string, StationsSelectionState> = cloneDeep(
      this.state.networkToStationsMap
    );
    const networkState: NetworkRow[] = cloneDeep(this.state.networkTableData);
    selectedRows.forEach(row => {
      const selectedNetworkStationStates: StationsSelectionState = this.getStationsSelectionState(
        row.id
      );
