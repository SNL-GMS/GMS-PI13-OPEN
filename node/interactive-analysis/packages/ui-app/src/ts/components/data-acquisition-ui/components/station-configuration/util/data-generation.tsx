...
export function generateDataSourcesData(): DataSourcesRow[] {
  ...
  for (let i = 0; i < maxRows; i++) {
    ...
    let idx = Math.floor((getSecureRandomNumber() * Object.keys(DataSources).length) / 2);
    const sourceName = DataSources[idx];
    idx = Math.floor((getSecureRandomNumber() * Object.keys(DataAcquisitionProtocols).length) / 2);
  ...
  return sortBy(dataSourcesRows, 'dataSource');
}
