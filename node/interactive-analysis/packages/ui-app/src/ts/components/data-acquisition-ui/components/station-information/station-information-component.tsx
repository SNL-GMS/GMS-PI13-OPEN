...
import { createDropdownItems, Table, TableApi } from '@gms/ui-core-components';
 ...
 */
export class StationInformation extends React.PureComponent<
  StationInformationProps,
  StationInformationState
> {
  ...
  public static generateTableData = (props: StationInformationProps): StationInformationRow[] => {
    ...
    const defaultStations = props.defaultStationsQuery
      ? props.defaultStationsQuery.defaultReferenceStations
      : undefined;
    if (defaultStations) {
      ...
      stationInformationListData = sortBy(stationInformationListData, 'station');
    }
    return stationInformationListData;
  }

  public static getDerivedStateFromProps(
    nextProps: StationInformationProps,
    prevState: StationInformationState
  ) {
    if (nextProps.defaultStationsQuery && nextProps.defaultStationsQuery.defaultReferenceStations) {
  ...
  public render() {
    ...
    return (
      <div className={classNames('ag-theme-dark', 'table-container-hide-overflow')}>
        <div className={classNames(Classes.HTML_SELECT, Classes.FILL, 'list-filter-select')}>
        ...
        <div className={'list-wrapper'}>
          <div className={'max'}>
            <Table
              id="table-station-information"
              key="table-station-information"
        ...
        <p />
        <ButtonGroup>
          <Button small={true} text="Add..." onClick={e => this.generateDefaultRow()} />
          <Popover
            content={
                ...
                >
                  <ButtonGroup>
                    ...
                    <div className={Classes.POPOVER_DISMISS}>
                      <Button small={true} text="Cancel" onClick={e => this.onImportCancel(e)} />
                    </div>
                  </ButtonGroup>
                  <br />
                  <br /> <br />
          ...
          >
            <Button small={true} text="Import CSS" />
          </Popover>
          <Popover
            content={
              <div className={'data-acquisition-popover'}>
                ...
                <br />
                <div
                  className={classNames(Classes.HTML_SELECT, Classes.FILL, 'list-filter-select')}
                >
                  <select
                    ...
                    onChange={(event: React.FormEvent<HTMLSelectElement>) => {
                      ...
                      this.setState({
                        popoverDataAcquisitionStatusValue: targetValue
                      });
                    }}
                  >
                ...
                >
                  <select
                    ...
                    onChange={(event: React.FormEvent<HTMLSelectElement>) => {
                      ...
                      this.setState({
                        popoverInteractiveProcessingStatusValue: targetValue
                      });
                    }}
                  >
                ...
                >
                  <select
                    ...
                    onChange={(event: React.FormEvent<HTMLSelectElement>) => {
                      ...
                      this.setState({
                        popoverAutomaticProcessingStatusValue: targetValue
                      });
                    }}
                  >
          ...
          <Button small={true} text="Select all" onClick={e => this.mainTable.selectAll()} />
          <div className={'list__button-row-divider-small'} />
          <Button small={true} text="Deselect all" onClick={e => this.cleanupAllRows()} />
          <div className={'list__button-row-divider-small'} />
          <Button small={true} text="Save" onClick={e => this.saveChanges()} />
          <div className={'list__button-row-divider-small'} />
          <Button small={true} text="Discard" onClick={e => this.discardChanges()} />
        </ButtonGroup>
      </div>
    );
  }
    ...
    switch (this.state.selectedFilter) {
      case 'Enabled':
        filteredList = data.filter(
          stationInformation => stationInformation.dataAcquisition === 'enabled'
        );
        break;
      case 'Disabled':
        filteredList = data.filter(
          stationInformation => stationInformation.dataAcquisition === 'disabled'
        );
        break;
      case 'All Stations':
        filteredList = data;
      ...
      case 'Interactive Processing - Available by default':
        filteredList = data.filter(
          stationInformation => stationInformation.interactiveProcessing === 'default'
        );
        break;
      case 'Interactive Processing - Available by request':
        filteredList = data.filter(
          stationInformation => stationInformation.interactiveProcessing === 'request'
        );
        break;
      case 'Station Processing stations':
        filteredList = data.filter(
          stationInformation => stationInformation.automaticProcessing === 'station'
        );
        break;
      case 'Network Processing stations':
        filteredList = data.filter(
          stationInformation => stationInformation.automaticProcessing === 'network'
        );
        break;
      case 'Disabled Processing stations':
        filteredList = data.filter(
          stationInformation => stationInformation.automaticProcessing === 'disabled'
        );
        break;
      default:
    }
  ...
  private readonly onRowClicked = (event: any) => {
    if (this.mainTable) {
      ...
      const allStationData: ReferenceStationTypes.ReferenceStation[] = this.props
        .defaultStationsQuery
        ? this.props.defaultStationsQuery.defaultReferenceStations
        : undefined;
      const selectedStation: ReferenceStationTypes.ReferenceStation = allStationData
        ? allStationData.filter(station => station.id === this.props.selectedStationIds[0])[0]
        : undefined;
      if (selectedStation) {
  ...
  private readonly popoverStationNamesTable = () => (
    ...
    >
      <Table
        id="table-station-information-popover"
        key="table-station-information-popover"
        context={{}}
        domLayout="autoHeight"
        rowData={this.generatePopoverTableData()}
        getRowNodeId={node => node.id}
        deltaRowDataMode={true}
  ...
  private readonly getSelectedStation = (e: any) => {
    ...
    const matchingStation: ReferenceStationTypes.ReferenceStation = this.state.stationsFromFiles.find(
      a => a.id === selectedStationName
    );
    this.setState({
  ...
  private readonly updateDataAcquisition = (id: string, newValue: DataAcquisitionStatus) => {
    ...
    const td: StationInformationRow[] = this.state.tableData.map(a => ({
      ...a
    }));
   ...
   * @param newValue new dropdown value
   */
  private readonly updateInteractiveProcessing = (
    id: string,
    newValue: InteractiveProcessingStatus
  ) => {
    const indexedValue = this.state.tableData.find(r => r.id === id);
    const td: StationInformationRow[] = this.state.tableData.map(a => ({
      ...a
   ...
   * @param newValue new dropdown value
   */
  private readonly updateAutomaticProcessing = (
    id: string,
    newValue: AutomaticProcessingStatus
  ) => {
    const indexedValue = this.state.tableData.find(r => r.id === id);
    const td: StationInformationRow[] = this.state.tableData.map(a => ({
      ...a
  ...
  private readonly updateTableWithDropdownValues = () => {
    ...
    this.props.selectedStationIds.forEach(stationId => {
      ...
      td.find(
        a => a.id === indexedRow.id
      ).dataAcquisition = this.state.popoverDataAcquisitionStatusValue;
      td.find(
        a => a.id === indexedRow.id
      ).interactiveProcessing = this.state.popoverInteractiveProcessingStatusValue;
      td.find(
        a => a.id === indexedRow.id
      ).automaticProcessing = this.state.popoverAutomaticProcessingStatusValue;
  ...
  private readonly importCssFiles = async (event: any) => {
    ...
    const sitechanFileContents = (await readUploadedFileAsText(
      this.state.sitechanFile
    )) as string[];

    const assembledStations = createProcessingStations(siteFileContents, sitechanFileContents);
    const stationsList = map(assembledStations, 'name').sort((a, b) => a.localeCompare(b, 'en'));
