...
export class StatusConfigurations extends React.PureComponent<
  StatusConfigurationsProps,
  StatusConfigurationsState
> {
  /**
   ...
   */
  private static readonly generateTableData = (
    props: StatusConfigurationsProps
  ): StatusConfigurationRow[] => {
  ...
  public static getDerivedStateFromProps(
    nextProps: StatusConfigurationsProps,
    prevState: StatusConfigurationsState
  ) {
    ...
    return (
      <div className={classNames('ag-theme-dark', 'table-container-hide-overflow')}>
        ...
        <div className={'list-wrapper'}>
          <div className={'max'}>
            <Table
              id="table-status-configuration"
              key="table-status-configuration"
              context={{
        ...
        <ButtonGroup>
          <Popover
            content={
              <div className={'table-container'}>
                ...
                {this.popoverDropdown(
                  this.state.popover.acquisitionValue,
                  Acquisition,
                ...
                {this.popoverDropdown(
                  this.state.popover.pkiInUseValue,
                  PkiInUse,
                  this.popoverPkiInUseOnChange
                )}
                <label>
                ...
                {this.popoverDropdown(
                  this.state.popover.processingPartitionValue,
                  ProcessingPartition,
                  this.popoverProcessingPartitionOnChange
                )}
                <label>
                  Store on acquisition partition
                  <br />
                </label>
                {this.popoverDropdown(
                  this.state.popover.storeOnAcquisitionParitionValue,
                  StoreOnAcquisitionPartition,
                  this.popoverStoreOnAcquisitionParitionOnChange
                )}
                <br />
          ...
          <Button small={true} text="Deselect all" onClick={e => this.mainTable.deselectAll()} />
          <div className={'list__button-row-divider-small'} />
    ...
    switch (this.state.selectedFilter) {
      ...
      case 'Acquire':
        filteredList = data.filter(
          statusConfiguration => statusConfiguration.acquisition === 'Acquire'
        );
        break;
      case "Don't Acquire":
        filteredList = data.filter(
          statusConfiguration => statusConfiguration.acquisition === "Don't Acquire"
        );
        break;
      case 'PKI in use':
        filteredList = data.filter(
          statusConfiguration => statusConfiguration.pkiInUse === 'Enabled'
        );
        break;
      case 'PKI not in use':
        filteredList = data.filter(
          statusConfiguration => statusConfiguration.pkiInUse === 'Disabled'
        );
        break;
      case 'Upload to Processing Partition':
        filteredList = data.filter(
          statusConfiguration => statusConfiguration.processingPartition === 'Upload'
        );
        break;
      case "Don't Upload to Processing Partition":
        filteredList = data.filter(
          statusConfiguration => statusConfiguration.processingPartition === "Don't upload"
        );
        break;
      case 'Store on Acquisition Partition':
        filteredList = data.filter(
          statusConfiguration => statusConfiguration.storeOnAcquisitionPartition === 'Store'
        );
        break;
      case "Don't Store on Acquisition Partition":
        filteredList = data.filter(
          statusConfiguration => statusConfiguration.storeOnAcquisitionPartition === "Don't store"
        );
        break;
      default:
    }
   ...
   * @param newValue new dropdown value
   */
  private readonly updateProcessingPartition = (
    stationId: string,
    newValue: ProcessingPartition
  ) => {
    const indexedValue = this.state.tableData.find(r => r.id === stationId);
    const td: StatusConfigurationRow[] = this.state.tableData.map(a => ({ ...a }));
   ...
   * @param newValue new dropdown value
   */
  private readonly updateStoreOnAcquisitionPartition = (
    stationId: string,
    newValue: StoreOnAcquisitionPartition
  ) => {
    const indexedValue = this.state.tableData.find(r => r.id === stationId);
    const td: StatusConfigurationRow[] = this.state.tableData.map(a => ({ ...a }));
  ...
  private readonly updateTableWithDropdownValues = () => {
    if (this.mainTable) {
      ...
      this.mainTable.getSelectedNodes().forEach(rowNode => {
        ...
        ).processingPartition = this.state.popover.processingPartitionValue;
        td.find(a => a.id === indexedRow.id).modified = true;
        td.find(
          a => a.id === indexedRow.id
        ).storeOnAcquisitionPartition = this.state.popover.storeOnAcquisitionParitionValue;
   ...
   */
  private readonly popoverDropdown = (
    selectValue: any,
    dropdownItems: any,
    dropdownOnChange: any
  ) => (
    <div className={classNames(Classes.HTML_SELECT, Classes.FILL, 'list-filter-select')}>
      <select value={selectValue} onChange={dropdownOnChange}>
  ...
  private readonly popoverEDCTable = () => (
    ...
    >
      <Table
        id="table-status-configuration-popover"
        key="table-status-configuration-popover"
   ...
   * @param event dropdown select event
   */
  private readonly popoverProcessingPartitionOnChange = (
   ...
   * @param event dropdown select event
   */
  private readonly popoverStoreOnAcquisitionParitionOnChange = (
    event: React.FormEvent<HTMLSelectElement>
  ) => {
