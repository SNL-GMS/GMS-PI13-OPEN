...
export function generateAcquisition(): Acquisition {
  ...
  const randomNumber = parseInt(
    Math.floor(getSecureRandomNumber() * (max - min + 1) + min).toFixed(),
    base
  );
...
export function generatePkiStatus(): PkiStatus {
  ...
  if (randomNumber === bound) {
    ...
    randomNumber % 2
      ? (pkiStatusValue = PkiStatus.INSTALLED)
      : (pkiStatusValue = PkiStatus.NOT_INSTALLED);
  }
...
export function generateStoreOnAcquisitionPartition(): StoreOnAcquisitionPartition {
  ...
  return randomNumber % 2
    ? StoreOnAcquisitionPartition.STORE
    : StoreOnAcquisitionPartition.DONT_STORE;
}
...
export function generateDataCenter(): boolean {
  ...
  const rand = getSecureRandomNumber() < 0.5;
  return rand;
}
...
export function generatePopoverEdcTableData(): PopoverEdcRow[] {
  ...
  dataCenters.forEach(dc => {
    const dataCenterRow: PopoverEdcRow = {
      ...
      dataCenter: dc.name,
      // tslint:disable-next-line: no-magic-numbers
      enabled: getSecureRandomNumber() < 0.5
    };
