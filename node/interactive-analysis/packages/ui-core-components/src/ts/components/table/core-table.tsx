...
> {
  ...
  /** Constructor */
  private constructor(props: CoreTableProps<RowDataType, ContextDataType>) {
    super(props);
  }
   ...
   */
  public componentDidUpdate(prevProps: CoreTableProps<RowDataType, ContextDataType>): void {
    if (this.props.debug) {
  ...
  public readonly refreshRenderedRowNodes = (): void => {
    if (this.tableApi) {
      ...
      const rowNodes: RowNode[] = [];
      this.tableApi.getRenderedNodes().forEach((node: RowNode, index: number) => {
        rowNodes.push(node);
      });
      this.tableApi.redrawRows({ rowNodes });
    }
  ...
  public readonly refreshAllRowNodes = (): void => {
    if (this.tableApi) {
      ...
      const rowNodes: RowNode[] = [];
      this.tableApi.forEachNode((node: RowNode, index: number) => {
        rowNodes.push(node);
      });
      this.tableApi.redrawRows({ rowNodes });
    }
  ...
  private readonly mapColumnDefinitionsForStaticRendering = (
    ...
    definitions
      ? // tslint:disable-next-line: cyclomatic-complexity
        cloneDeep(this.props.columnDefs).map(definition => {
          if (
