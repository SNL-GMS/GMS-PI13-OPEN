  ...
  private constructor(props) {
    ...
    this.state = {
      ...
      displayString: this.props.shortFormat
        ? dateToShortISOString(this.props.date)
        : dateToISOString(this.props.date),
        ...
        <textarea
          ...
          onChange={e => {
            ...
            const newDate = this.props.shortFormat
              ? parseShortISOString(e.target.value)
              : parseISOString(e.target.value);
            // If the date is valid
            if (validStringFormat && !isNaN(newDate.getTime())) {
              this.setState({ ...this.state, isValid: true, displayString: e.target.value }, () => {
            ...
            onChange={(inputDate, isUserChange) => {
              ...
              if (isUserChange) {
                ...
                this.setState({
                  ...
                  showDatePicker: false,
                  displayString: this.props.shortFormat
                    ? dateToShortISOString(newDate)
                    : dateToISOString(newDate)
                });
              }
            }}
          />
