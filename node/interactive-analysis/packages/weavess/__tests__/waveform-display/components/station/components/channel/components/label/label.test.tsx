...
export function generateConfiguration(): Entities.Configuration {
  return {
    ...
    defaultChannelHeightPx: random.arrayElement([undefined, random.number({ min: 1, max: 100 })]),
    labelWidthPx: random.arrayElement([undefined, random.number({ min: 1, max: 100 })]),
...
export function generateHotKeyConfiguration() {
  return {
    ...
    amplitudeScaleSingleReset: random.arrayElement([undefined, random.alphaNumeric(1)]),
    amplitudeScaleReset: random.arrayElement([undefined, random.alphaNumeric(1)]),
...
export function generateChannelConfiguration() {
  return {
    ...
    disableSignalDetectionModification: random.arrayElement([undefined, random.boolean()]),
...
export function generateChannelSegments(): Map<string, Entities.ChannelSegment> {
...
export function generateDisplayType(): Entities.DisplayType[] {
  ...
  for (let i = 0; i < random.number({ min: 1, max: 3 }); i += 1) {
    displayTypes.push(Entities.DisplayType[random.objectElement(Entities.DisplayType)]);
  }
  return displayTypes;
}
...
export function generateDataSegment(): Entities.DataSegment {
  ...
  return {
    data: {
      startTimeSecs: date.recent().getUTCMilliseconds(),
...
export function generateLabelProps(): LabelProps {
  return {
    ...
    distanceUnits: Entities.DistanceUnits[random.objectElement(Entities.DistanceUnits)],
    events: undefined,
...
describe('Label Tests', () => {
  ...
  test('mount should be able to offer click response', (done: jest.DoneCallback) => {
    ...
    setImmediate(() => {
      ...
      expect(containerStyle).toHaveProperty('textShadow', 'initial');

      // tslint:disable-next-line: no-unnecessary-type-assertion
      expect((wrapper.prop('selections') as any).channels).toHaveLength(0);
      ...
      // tslint:disable-next-line: no-unnecessary-type-assertion
      expect((wrapper.prop('selections') as any).channels).toHaveLength(1);

      containerStyle = labelContainer.get(0).props.style;
      expect(containerStyle).toHaveProperty('textShadow', '0px 1px 15px');
