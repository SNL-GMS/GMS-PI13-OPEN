...
export class RecordSection extends React.Component<{}, RecordSectionState> {
  ...
  public update(clear: boolean) {
    ...
    if (!clear && this.state.options.data.length !== 0) {
      this.loadData(this.state.options.data).catch();
    } else {
      // this.state.topVal = 0;
  ...
  public async loadData(waveformArray: any[]) {
    ...
    for (const waveform of waveformArray) {
      ...
      const float32Array: Float32Array = await this.workerRpc.rpc<{}, Float32Array>(
      ...
      const detectionTimeSecs = waveform.signalDetection[0].time.getTime() / MILLISECONDS_IN_SECOND;
