...
export class Channel extends React.PureComponent<ChannelProps, ChannelState> {
   ...
   */
  public renderScene = (renderer: THREE.WebGLRenderer, boundsRect: ClientRect | DOMRect) => {
  ...
  public readonly updateScrollPosition = (scrollWidth: number, scrollLeft: number) => {
    ...
    const descriptionLabelRightPosition = `calc(${scrollWidth}px - ${clientWidth}px - ${scrollLeft}px - ${DEFAULT_LABEL_WIDTH_PIXELS}px + 6px)`;
  ...
  private readonly renderContent = (): React.ReactFragment => {
    ...
    return waveform || spectrogram ? (
      ...
      <React.Fragment>{this.renderNoGraphics()}</React.Fragment>
    );
  }
          ...
          >
            <ContentRenderer
              ...
              descriptionLabelColor={
                channelSegment ? channelSegment.descriptionLabelColor : undefined
              }
          ...
          >
            <ContentRenderer
              ...
              theoreticalPhaseWindows={
                spectrogram ? spectrogram.theoreticalPhaseWindows : undefined
              }
  ...
  private readonly onMeasureWindowClick = (e: React.MouseEvent<HTMLDivElement>): void => {
    ...
    const startDivLeft =
      wMeasureWindowSelectionAreaRef && wMeasureWindowSelectionAreaRef.style.left
        ...
        ? parseFloat(sMeasureWindowSelectionAreaRef.style.left)
        : 0;
    ...
    const startDivRight =
      wMeasureWindowSelectionAreaRef && wMeasureWindowSelectionAreaRef.style.right
        ...
        ? parseFloat(sMeasureWindowSelectionAreaRef.style.right)
        : 0;
    ...
    const onMouseUp = (event: MouseEvent) => {
      ...
      if (this.props.updateMeasureWindow) {
        this.props.updateMeasureWindow(
          ...
          this.removeMeasureWindowSelection
        );
      }
      ...
      if (isMeasureWindowDisabled) {
        ...
        if (this.props.updateMeasureWindow) {
          ...
          const scale = d3
            .scaleLinear()
            .domain([0, 1])
            .range([this.props.getViewRange()[0], this.props.getViewRange()[1]]);
          ...
          const onMouseUp = (event: MouseEvent) => {
            ...
            isDragging = false;
            const startTimeSecs = this.props.computeTimeSecsForMouseXPosition(
              Math.min(start, this.mouseXPosition)
            );
            const endTimeSecs = this.props.computeTimeSecsForMouseXPosition(
              Math.max(start, this.mouseXPosition)
            );
            if (this.props.updateMeasureWindow) {
              this.props.updateMeasureWindow(
                this.props.stationId,
  ...
  private readonly onWaveformKeyDown = (e: React.KeyboardEvent<HTMLDivElement>): void => {
    if (!e.repeat) {
      if (this.props.configuration.hotKeys.amplitudeScaleSingleReset) {
        if (
          isHotKeyCommandSatisfied(
            e.nativeEvent,
            this.props.configuration.hotKeys.amplitudeScaleSingleReset
          )
        ) {
          this.resetAmplitude();
        }
      ...
      if (this.props.configuration.hotKeys.amplitudeScale) {
        if (
          isHotKeyCommandSatisfied(e.nativeEvent, this.props.configuration.hotKeys.amplitudeScale)
        ) {
  ...
  private readonly onKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    if (!e.repeat) {
      if (this.waveformContentRef) {
        // ! TODO handle waveform and spectrogram
  ...
  private readonly internalRenderScene = (
    ...
    if (!renderer || !boundsRect || !scene || !camera || !container) return;
