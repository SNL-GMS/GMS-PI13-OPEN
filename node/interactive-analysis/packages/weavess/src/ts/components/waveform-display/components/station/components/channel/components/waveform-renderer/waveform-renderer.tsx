...
} from '../../../../../../../../workers/create-position-buffer';
import {
...
import { isDataBySampleRate, isDataByTime } from './util';
...
> {
  ...
  /** Flag that indicates the amplitude is being adjusted */
      ...
      this.props.channelSegments.forEach((channelSegment: Entities.ChannelSegment, key: string) => {
        ...
        if (channelSegment.dataSegments) {
          channelSegment.dataSegments.forEach(dataSegment => {
            const float32Array = isDataBySampleRate(dataSegment.data)
              ? createPositionBufferForDataBySampleRate({
                  data: dataSegment.data.values,
        ...
        if (channelSegment && channelSegment.dataSegments) {
          ...
          channelSegment.dataSegments.forEach(dataSegment => {
            const values = isDataBySampleRate(dataSegment.data)
              ? dataSegment.data.values
              : isDataByTime(dataSegment.data)
            ...
            if (values.length > 0) {
              ...
              segmentAvg = segmentAvg / values.length;
  ...
  private readonly renderChannelMasks = (masks: Entities.Mask[]) => {
    ...
    sortBy(masks, (mask: Entities.Mask) => mask.endTimeSecs - mask.startTimeSecs).forEach(
      (mask, i, arr) => {
