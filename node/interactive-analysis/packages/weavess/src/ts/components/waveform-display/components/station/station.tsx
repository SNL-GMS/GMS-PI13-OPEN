...
import {
  isDataBySampleRate,
  isDataByTime
} from './components/channel/components/waveform-renderer/util';
...
export class Station extends React.PureComponent<StationProps, StationState> {
  ...
  public render() {
    ...
    rowHeights.push(
      this.props.station.defaultChannel.height ||
        this.props.configuration.defaultChannelHeightPx ||
        DEFAULT_CHANNEL_HEIGHT_PIXELS
    );
        ...
        <Channel // default channel
          ...
          distanceUnits={
            this.props.station.distanceUnits
              ? this.props.station.distanceUnits
              : Entities.DistanceUnits.degrees
          }
          events={
          ...
          onMouseDown={(
            ...
            this.props.onMouseDown(
              ...
              xPct,
              channelId,
              timeSecs - defaultChannelTimeOffsetSeconds,
              isDefaultChannel
            )
          }
          onMouseUp={(
                ...
                <Channel // Channel (for non-default channels)
                  ...
                  events={
                    this.props.events && this.props.events.nonDefaultChannelEvents
                      ? this.mapEventsToOffset(
                          channelchannel,
                          this.props.events.nonDefaultChannelEvents
                        )
                      : undefined
                  }
                  canvasRef={this.props.canvasRef}
   ...
   * label to always be in view and aligned to the left.
   */
  ...
  private readonly mapChannelConfigToOffset = (channel: Entities.Channel): Entities.Channel => {
    ...
    const timeOffsetSeconds: number = channel.timeOffsetSeconds;
      ...
      channel.waveform && channel.waveform.markers
        ? {
            ...
            selectionWindows: channel.waveform.markers.selectionWindows
              ? channel.waveform.markers.selectionWindows.map(s => ({
                  ...s,
                  startMarker: {
                    ...s.startMarker,
    ...
    const spectrogramPredictedPhases: Entities.PickMarker[] | undefined =
      ...
      channel.spectrogram && channel.spectrogram.markers
        ? {
            verticalMarkers: channel.spectrogram.markers.verticalMarkers
              ? channel.spectrogram.markers.verticalMarkers.map((v: Entities.Marker) => ({
                  ...v,
                  timeSecs: v.timeSecs + timeOffsetSeconds
                }))
              : undefined,
  ...
  private readonly mapEventsToOffset = (
    channel: Entities.Channel,
    channelEvents: Entities.ChannelEvents
  ): Entities.ChannelEvents => {
    if (!channel.timeOffsetSeconds) {
