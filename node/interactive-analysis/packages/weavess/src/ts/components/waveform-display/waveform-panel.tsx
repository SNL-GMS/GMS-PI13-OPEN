...
const workerRpcs = range(window.navigator.hardwareConcurrency || defaultNumWorkers).map(_ => {
...
export class WaveformPanel extends React.PureComponent<WaveformPanelProps, WaveformPanelState> {
  ...
  private waveformsViewportScrollTop: number = 0;
  ...
  public static getDerivedStateFromProps(
    nextProps: WaveformPanelProps,
    prevState: WaveformPanelState
  ) {
    return null; /* no-op */
  }
  ...
  public render() {
    ...
    return (
      ...
      >
              ...
              <div className="weavess-wp-container-3">
                <div
                  ...
                  onScroll={e => {
                    ...
                    if (this.ctrlKey) {
                      this.waveformsViewportRef.scroll({ top: this.waveformsViewportScrollTop });
                    } else {
                      this.waveformsViewportScrollTop = this.waveformsViewportRef.scrollTop;
                    }
                    ...
                    const labelWidthPx =
                      this.props.configuration.labelWidthPx || DEFAULT_LABEL_WIDTH_PIXELS;
                    const timeRangeLeft =
                      viewport.scrollLeft / (viewportContentContainer.clientWidth - labelWidthPx);
                    // tslint:disable-next-line:max-line-length
                    const timeRangeRight =
                      (viewport.scrollLeft + this.canvasRef.clientWidth) /
                      (viewportContentContainer.clientWidth - labelWidthPx);
        ...
        >
          <span ref={ref => (this.timeRangeRef = ref)} />
        </div>
      </div>
    );
  }
  ...
  private readonly onKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    const amplitudeScaleSingleResetHotKey = this.props.configuration.hotKeys
      ...
      stationElements.push(
        <Station
          ...
          ref={stationRef => {
            ...
            if (stationRef) {
              this.stationComponentRefs.push(stationRef);
            }
          }}
  ...
  private readonly displayCurrentTimeRange = () => {
    ...
    this.timeRangeRef.innerHTML = `${moment
      .unix(left)
      .utc()
  ...
  private readonly renderWaveforms = (): void => {
    window.requestAnimationFrame(() => {
      ...
      if (
        ...
        this.waveformsViewportRef.clientWidth === 0
      ) {
        return;
      }
      ...
      this.stationComponentRefs.forEach(station => {
        if (this.waveformsViewportRef) {
          station.updateScrollPosition(
            this.waveformsViewportRef.scrollWidth,
            this.waveformsViewportRef.scrollLeft
          );
        }
      });
        ...
        if (events) {
          if (events.onMaskCreateDragEnd) {
            const channels = flatMap(
              this.props.stations.map<Entities.Channel[]>((s: Entities.Station) =>
  ...
  private readonly hasReachedMaxZoomLevel = (): boolean => {
    ...
    return (
      parseFloat(this.viewRange[1].toFixed(4)) - parseFloat(this.viewRange[0].toFixed(4)) <=
      maxZoomDiff
    );
  }
  ...
  private readonly zoom = (start: number, end: number) => {
    ...
    if (this.viewRange[0] !== start || this.viewRange[1] !== end) {
      if (start === 0 && end === 1) {
