...
function explain_basic_usage {
  ...
  [ $? == 1 ] && exit 1
  echo "#############"
  echo "Mock Producer Run Help"
  echo "Runs the mock producer to put data on the kafka topics for the UI to consume."
...
function print_options {
  ...
  echo "  --data-dir                                 Directory path to location of mock data i.e.: 'repos/<gitlab-url>/gms/gms-common/node/interactive-analysis/scripts/mock-producer/mockdata/'."
  ...
  echo "  -m | --mount                               The name of the docker volume you would like to use to mount mock data into the mock producer. i.e. 'gms-data'"
  echo "  -v | --verbose  "
...
function init_producer {
  base_directory=$GMS_COMMON_DIR/node/interactive-analysis/scripts/mock-producer/mockdata
...
function start_producer {
  ...
  pushd $GMS_COMMON_DIR/java/gms/shared/frameworks/data-injector
    cmd="docker run --rm ${debug_options} $detached_flag -it -v $pathToData \
      --network $network \
      local/gms-common/frameworks-data-injector:latest \
...
function validate_is_built {
  validate_image_exists "local/gms-common/frameworks-data-injector"
